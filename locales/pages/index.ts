// Common translations (nav, hero, work, about, cta, footer)
export { commonEn, commonZh } from './common'

// Project page translations
export { idoCmsEn, idoCmsZh } from './ido-cms'
export { tgballersEn, tgballersZh } from './tgballers'
export { ivarRedesignEn, ivarRedesignZh } from './ivar-redesign'
export { faceLibraryEn, faceLibraryZh } from './face-library'
export { dataVisualizationEn, dataVisualizationZh } from './data-visualization'
export { idoWorkflowEn, idoWorkflowZh } from './ido-workflow'
export { pankoMascotEn, pankoMascotZh } from './panko-mascot'
export { videoAnalysisEn, videoAnalysisZh } from './video-analysis'
export { vodChannelEn, vodChannelZh } from './vod-channel'
export { chromecastEn, chromecastZh } from './chromecast'
export { otterlyEn, otterlyZh } from './otterly'
